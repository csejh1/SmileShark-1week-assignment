...
# 수정 후 — 첫 요청 시 조회 (Envoy 초기화 완료 후)
server {
    resolver 169.254.169.253 valid=10s ipv6=off;
    location /petclinic/ {
        set $backend http://was:9966;
        proxy_pass $backend;
    }
}
```

`resolver 169.254.169.253`은 AWS VPC 내부 DNS 서버. nginx가 요청을 받는 시점에는 Envoy가 이미 초기화 완료 상태이므로 `was` 이름 해석이 정상 동작함.
