...
export default class PetEditor extends React.Component<IPetEditorProps, IPetEditorState> {
  ...
  onSubmit(event) {
    ...
    const typeId = Number(editablePet.typeId);
    const selectedPetType = this.props.pettypes.find(pt => Number(pt.value) === typeId);
    ...
    const request: IPetRequest = {
      ...
      type: {
        id: typeId,
        name: selectedPetType ? selectedPetType.name : ''
      }
    };
